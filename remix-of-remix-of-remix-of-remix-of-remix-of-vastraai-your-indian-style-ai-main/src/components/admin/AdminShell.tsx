    ...
    return (
      <div className="min-h-screen grid place-items-center bg-[#0c0a09] text-amber-100 p-6">
        <div className="max-w-md w-full text-center border border-amber-500/20 rounded-2xl p-8 bg-[#161210] shadow-[0_0_60px_-20px_rgba(217,168,93,0.4)]">
          ...
          <h1 className="font-display text-3xl mt-4">VastraAI Admin</h1>
          <p className="text-sm mt-2 text-amber-100/60">
            Your account doesn't have admin access yet. If you're the owner, claim it once below.
          </p>
          <button
            onClick={claim}
            className="mt-6 w-full bg-gradient-to-r from-amber-500 to-yellow-400 text-[#1a0f08] font-semibold py-3 rounded-full hover:brightness-110 transition"
          >
            Claim admin access
          </button>
          <Link to="/" className="block mt-4 text-xs text-amber-200/50 hover:text-amber-200">← Back to store</Link>
        </div>
      </div>
    );
  }
