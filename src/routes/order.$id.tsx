...
import { getOrder, deriveStatus, mapDbOrderToOrder, type Order } from "@/lib/orders";
  ...
  useEffect(() => {
    async function loadOrder() {
      ...
      try {
        ...
        if (dbOrder && !error) {
          setOrder(mapDbOrderToOrder(dbOrder));
          return;
        }
      } catch (err) {
        console.error("Failed to query order from Supabase:", err);
      }
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-xl px-4 py-20 text-center">
          ...
          <p className="text-muted-foreground mt-2">We couldn't find order #{id}. Make sure you copied the correct link or order ID.</p>
        ...
        <section className="mt-10 bg-card border rounded-3xl p-6 md:p-8">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="text-sm text-muted-foreground">
              {status.stage >= 0 ? `ETA ${new Date(order.estimatedDelivery).toLocaleDateString()}` : "Status updated"}
            </div>
          </div>
          {status.stage < 0 ? (
            <div className="mt-6 flex flex-col items-center justify-center py-6 text-center bg-red-950/10 border border-red-900/20 rounded-2xl p-6">
              ...
              <h3 className="font-display text-xl font-bold text-red-400 mt-2">Order {status.label}</h3>
              <p className="text-sm text-muted-foreground mt-1 max-w-lg">This order has been cancelled or returned. If you require further assistance or have queries regarding refunds, please contact our support desk.</p>
            </div>
          ) : (
            <div className="mt-8 flex justify-between relative">
              <div className="absolute top-5 left-5 right-5 h-0.5 bg-muted"/>
              <div className="absolute top-5 left-5 h-0.5 bg-gold-grad transition-all" style={{ width: `calc(${((status.stage >= 0 ? status.stage : 0) / (stages.length - 1)) * 100}% - ${status.stage === stages.length-1 ? '40px' : '0px'})` }}/>
              {stages.map((s, i) => (
